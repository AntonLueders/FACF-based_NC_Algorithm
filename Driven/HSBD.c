...
int main(int argc, char *argv[]) {
	...
	int resolution, min_rate, n_min, n_max, t_samp;
    double a, e;
    ...
    double ****f0;
	double ****n0;
    InitStorageArraysFACF(n_max, rateF, N, dim, &counter_FACF, &facf, &facf2, &nfcf, &f0, &n0);	
	...
	for (int step = 0; step < steps; step++) {
		...
		// Interaction forces are calculated in CollsionAvoidance()!		
		...
		if(step > EQ) {  // Equilibration time
			CalcFACF(n_max, sample, lengthF, rateF, facf, facf2, nfcf, f0, n0, counter_FACF,
				P, &t_samp); // See calc.c
        }
	...
	PrintFACF(facf_file, n_max, lengthF, facf, facf2, nfcf, counter_FACF); // See printdata.c
	fclose(facf_file);
	...
	FreeVerLists(P);
	FreeMemoryFACF(n_max, sample, lengthF, rateF, facf, facf2, nfcf, f0, n0);
